...
def beta_gamma_not_in_range(beta, gamma, leg_name,BETA_MAX_ANGLE=360,GAMMA_MAX_ANGLE=360):
...
def might_sanity_leg_lengths_check(hexapod, leg_name, points,ASSERTION_ENABLED=True):
...
def might_sanity_beta_gamma_check(beta, gamma, leg_name, points,ASSERTION_ENABLED=True):
    if not ASSERTION_ENABLED:
        return
...
def might_print_ik(poses, ik_parameters, hexapod,PRINT_IK=True):
    if not PRINT_IK:
        return
...
def might_print_points(points, leg_name,PRINT_IK_LOCAL_LEG=True):
