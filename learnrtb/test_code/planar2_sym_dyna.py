...
class Planar2(DHRobot):
    ...
    def __init__(self, symbolic=True):
        ...
        rl1, rl2 = sym.symbol("rl1 rl2")
        r0 = [zero, zero, zero] # center of mass position with respect to ith link
        ...
        I0=[zero, zero, zero, zero, zero, zero]
        ...
        L = [\
            RevoluteMDH(a=a0, alpha=zero,r=r0,m=m0,I=I0,Jm=zero,G=zero,B=zero,Tc=[zero,zero]),\
            RevoluteMDH(a=a0, alpha=zero,r=r1,m=m1,I=I1,Jm=zero,G=zero,B=zero,Tc=[zero,zero]),\
            RevoluteMDH(a=a1, alpha=zero,r=r2,m=m2,I=I2,Jm=zero,G=zero,B=zero,Tc=[zero,zero]),\
            RevoluteMDH(a=a2, alpha=zero,Jm=zero,G=zero,B=zero,Tc=[zero,zero])
            ]

        super().__init__(L, symbolic=True,name="Planar 2 link", keywords=("planar",))

        self.qr = [pi/4, pi/4, pi/4,zero]
        self.qz = [zero, zero, zero,zero]
...
def symbolic_dyna():
    ...
    thetas = [zero,theta1,theta2,zero]
    thetads = [zero,thetad1,thetad2,zero]
    thetadds = [zero,thetadd1,thetadd2,zero]
