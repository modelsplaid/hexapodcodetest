...
# [GCC 9.4.0]
# Embedded file name: /media/a/42d20e5a-ecfb-43c5-99d3-34fc18ecd184/yihang/pc_share/HexaClean/mdh_raspi_hexapod/tzq_raspi_hexapod/custom_type/bot_cart_type.py
# Compiled at: 2024-01-03 13:56:30
# Size of source mod 2**32: 14579 bytes
import sys
sys.path.append('../')
import numpy as np
...
class BotCartType(BotBaseType):
    ...
    def __init__(self, name='', cart_dic=None):
        ...
        if cart_dic is not None and not isinstance(cart_dic, dict) == False:
            ...
            cart_tmplt = {0:{
              'x': 0, 'y': 0, 'z': 0, 'name': "'right-middle'", 'id': 0}, 
             1:{
              'x': 0, 'y': 0, 'z': 0, 'name': "'right-front'", 'id': 1}, 
             2:{
              'x': 0, 'y': 0, 'z': 0, 'name': "'left-front'", 'id': 2}, 
             3:{
              'x': 0, 'y': 0, 'z': 0, 'name': "'left-middle'", 'id': 3}, 
             4:{
              'x': 0, 'y': 0, 'z': 0, 'name': "'left-back'", 'id': 4}, 
             5:{
              'x': 0, 'y': 0, 'z': 0, 'name': "'right-back'", 'id': 5}}
            self.ct_nam_arr = [
             'x', 'y', 'z']
            if cart_dic == None:
    ...
    def transf_lft_pts(self, rxyz_deg=[
 0, 0, 0], txyz=[0, 0, 0]):
        """
...
def test():
    ...
    for i in range(50):
        cdic_tmplt = {'0':{'x':19 * random(), 
          'y':19 * random(),  'z':19 * random(),  'name':'right-middle',  'id':0}, 
         '1':{'x':26 * random(), 
          'y':26 * random(),  'z':26 * random(),  'name':'right-front',  'id':1}, 
         '2':{'x':13 * random(), 
          'y':13 * random(),  'z':13 * random(),  'name':'left-front',  'id':2}, 
         '3':{'x':20 * random(), 
          'y':20 * random(),  'z':20 * random(),  'name':'left-middle',  'id':3}, 
         '4':{'x':25 * random(), 
          'y':25 * random(),  'z':25 * random(),  'name':'left-back',  'id':4}, 
         '5':{'x':15 * random(), 
          'y':15 * random(),  'z':15 * random(),  'name':'right-back',  'id':5}}
        ct1 = BotCartType()
...
def test_transform():
    dic_tmplt = {0:{
      'x': 1, 'y': 2, 'z': 3, 'name': "'right-middle'", 'id': 0}, 
     1:{
      'x': 1, 'y': 2, 'z': 3, 'name': "'right-front'", 'id': 1}, 
     2:{
      'x': 1, 'y': 2, 'z': 3, 'name': "'left-front'", 'id': 2}, 
     3:{
      'x': 1, 'y': 2, 'z': 3, 'name': "'left-middle'", 'id': 3}, 
     4:{
      'x': 1, 'y': 2, 'z': 3, 'name': "'left-back'", 'id': 4}, 
     5:{
      'x': 1, 'y': 2, 'z': 3, 'name': "'right-back'", 'id': 5}}
...
def test_loop():
    dic_tmplt = {0:{
      'x': 1, 'y': 2, 'z': 3, 'name': "'right-middle'", 'id': 0}, 
     1:{
      'x': 12, 'y': 22, 'z': 32, 'name': "'right-front'", 'id': 1}, 
     2:{
      'x': 13, 'y': 23, 'z': 33, 'name': "'left-front'", 'id': 2}, 
     3:{
      'x': 14, 'y': 24, 'z': 34, 'name': "'left-middle'", 'id': 3}, 
     4:{
      'x': 16, 'y': 26, 'z': 36, 'name': "'left-back'", 'id': 4}, 
     5:{
      'x': 17, 'y': 27, 'z': 37, 'name': "'right-back'", 'id': 5}}
    ct1 = BotCartType('ct1', dic_tmplt)
